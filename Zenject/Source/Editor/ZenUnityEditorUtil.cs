...
namespace Zenject.Internal
{
    public static class ZenUnityEditorUtil
    {
        ...
        public static void ValidateCurrentSceneSetup()
        {
            ...
            if (encounteredError)
            {
                throw new ZenjectException(
                    "Zenject Validation Failed!  See errors below for details."
                );
            }
        }
        ...
        {
            var activeScenePaths = EditorBuildSettings
                .scenes.Where(x => x.enabled)
                .Select(x => x.path)
                .ToList();
        ...
        public static void RunCurrentSceneSetup()
        {
            ...
            foreach (var sceneContext in GetAllSceneContexts())
            {
                ...
                catch (Exception e)
                {
                    ...
                    throw new ZenjectException(
                        "Scene '{0}' Failed To Start!".Fmt(sceneContext.gameObject.scene.name),
                        e
                    );
                }
            }
        ...
        public static SceneContext GetSceneContextForScene(Scene scene)
        {
            ...
            Assert.IsNotNull(
                sceneContext,
                "Could not find scene context for scene '{0}'",
                scene.name
            );
        ...
        public static SceneContext TryGetSceneContextForScene(Scene scene)
        {
            ...
            Assert.That(
                sceneContexts.Count == 1,
                "Found multiple SceneContexts in scene '{0}'.  Expected a maximum of one.",
                scene.name
            );
        ...
        public static SceneDecoratorContext GetDecoratorContextForScene(Scene scene)
        {
            ...
            Assert.IsNotNull(
                decoratorContext,
                "Could not find decorator context for scene '{0}'",
                scene.name
            );
        ...
        public static SceneDecoratorContext TryGetDecoratorContextForScene(Scene scene)
        {
            ...
            Assert.That(
                decoratorContexts.Count == 1,
                "Found multiple DecoratorContexts in scene '{0}'.  Expected a maximum of one.",
                scene.name
            );
        ...
        static IEnumerable<SceneContext> GetAllSceneContexts()
        {
            ...
            for (int i = 0; i < EditorSceneManager.sceneCount; i++)
            {
                ...
                {
                    Assert.That(
                        !string.IsNullOrEmpty(decoratorContext.DecoratedContractName),
        ...
        {
            return Path.Combine(
                Path.Combine(Path.GetFullPath(Application.dataPath), ".."),
                assetPath
            );
        }
            ...
            foreach (var item in selectedAssets)
            {
                ...
                if (!string.IsNullOrEmpty(relativePath))
                {
                    var fullPath = Path.GetFullPath(
                        Path.Combine(Application.dataPath, Path.Combine("..", relativePath))
                    );
