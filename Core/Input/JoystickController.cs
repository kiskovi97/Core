...
namespace Kiskovi.Core
{
            ...
            IDragHandler
    {
        ...
        [Range(0, 16)]
        [Tooltip(
            "Set 0 or 1 for free handle, or set according to number of direction need like 4 or 8"
        )]
        public int directionSnaps = 0;

        [Tooltip(
            "if true, snap handle to center when not using. Use false only when a certain direction is always needed."
        )]
        ...
        {
            if (baseMode == JoystickBaseMode.Static && !dragStartedInside)
                return;
        ...
        private void SendVector(Vector2 direction)
        {
            if (CachedType != null && typeof(InputVector2Signal).IsAssignableFrom(CachedType))
            {
                var instance = Activator.CreateInstance(CachedType, direction);
        ...
        void OnDrawGizmos()
        {
            if (joystickBase != null)
            {
                ...
                Handles.DrawLine(
                    baseWorldPos,
                    baseWorldPos + ((Vector3)inputDirection * joystickRange)
                );
            }
        }
#endif
